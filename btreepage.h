...
size_t binary_search(Container &container, size_t first, size_t last,
                     ObjType &object) {
  if (first >= last)
    return first;
  while (first < last) {
...
template <typename Container> void remove(Container &container, size_t pos) {
  size_t size = container.size();
  for (auto i = pos + 1; i < size; i++)
    container[i - 1] = container[i];
}

template <typename keyType, typename ObjIDType> struct tagObjectInfo {
  keyType key;
  ObjIDType ObjID;
  size_t UseCounter;
...
// this is the in-memory version of the CBTreePage
{
  ...
  typedef void (*lpfnForEach3)(ObjectInfo &info, size_t level, void *pExtra1,
                               void *pExtra2);

  // TODO create firstthat (foreach.h)
  typedef ObjectInfo *(*lpfnFirstThat2)(ObjectInfo &info, size_t level,
                                        void *pExtra1);
  typedef ObjectInfo *(*lpfnFirstThat3)(ObjectInfo &info, size_t level,
                                        void *pExtra1, void *pExtra2);

public:
  CBTreePage(size_t maxKeys, bool unique = true);
  virtual ~CBTreePage();
  // TODO replace 2nd param by LinkedValueType value
  ...
  ObjectInfo *FirstThat(lpfnFirstThat3 lpfn, size_t level, void *pExtra1,
                        void *pExtra2);

  // ====================================================================
  // ACCESORES: Para uso de iteradores en BTree
  // ====================================================================
  BTPage *getSubPage(size_t pos) {
    return (pos <= m_KeyCount) ? m_SubPages[pos] : nullptr;
  }
  ObjectInfo &getKey(size_t pos) { return m_Keys[pos]; }

  // ====================================================================
  // VARIADIC INORDER: Recorrido inorder con variadic templates
  // ====================================================================
  template <typename Function, typename... Args>
  void inorder(Function func, size_t level, Args &...args) {
    for (size_t i = 0; i < m_KeyCount; i++) {
      if (m_SubPages[i])
        m_SubPages[i]->inorder(func, level + 1, args...);
      func(m_Keys[i], level, args...);
    }
    if (m_SubPages[m_KeyCount])
      m_SubPages[m_KeyCount]->inorder(func, level + 1, args...);
  }

  // ====================================================================
  // VARIADIC PREORDER: Recorrido preorder con variadic templates
  // ====================================================================
  template <typename Function, typename... Args>
  void preorder(Function func, size_t level, Args &...args) {
    for (size_t i = 0; i < m_KeyCount; i++) {
      func(m_Keys[i], level, args...);
      if (m_SubPages[i])
        m_SubPages[i]->preorder(func, level + 1, args...);
    }
    if (m_SubPages[m_KeyCount])
      m_SubPages[m_KeyCount]->preorder(func, level + 1, args...);
  }

  // ====================================================================
  // VARIADIC POSTORDER: Recorrido postorder con variadic templates
  // ====================================================================
  template <typename Function, typename... Args>
  void postorder(Function func, size_t level, Args &...args) {
    for (size_t i = 0; i <= m_KeyCount; i++) {
      if (m_SubPages[i])
        m_SubPages[i]->postorder(func, level + 1, args...);
      if (i < m_KeyCount)
        func(m_Keys[i], level, args...);
    }
  }
  ...
  // ====================================================================
  // VARIADIC FIRSTTHAT: Búsqueda con predicado variadic
  // ====================================================================
  ...
  // ====================================================================
  // WRITE: Escribe la página al stream
  // ====================================================================
  void Write(ostream &os) {
    // Escribir claves de esta página
    for (size_t i = 0; i < m_KeyCount; i++) {
      os << m_Keys[i].key << " " << m_Keys[i].ObjID << " ";
    }
    // Escribir hijos recursivamente
    for (size_t i = 0; i <= m_KeyCount; i++) {
      if (m_SubPages[i])
...
private:
  ...
  void MovePage(BTPage *pChildPage, vector<ObjectInfo> &tmpKeys,
...
template <typename Trait> CBTreePage<Trait>::~CBTreePage() { Reset(); }

template <typename Trait>
bt_ErrorCode CBTreePage<Trait>::Insert(const keyType &key,
...
template <typename Trait> bool CBTreePage<Trait>::Redistribute1(size_t &pos) {
...
template <typename Trait> bool CBTreePage<Trait>::Redistribute2(size_t pos) {
  assert(pos > 0 && pos < NumberOfKeys());
  assert(m_SubPages[pos - 1] != 0 && m_SubPages[pos] != 0 &&
         m_SubPages[pos + 1] != 0);
  assert(m_SubPages[pos - 1]->Underflow() || m_SubPages[pos]->Underflow() ||
         m_SubPages[pos + 1]->Underflow());

  if (m_SubPages[pos - 1]->Underflow()) { // Rotate R2L
    RedistributeR2L(pos + 1);
    RedistributeR2L(pos);
    if (m_SubPages[pos - 1]->Underflow())
      return false;
  } else if (m_SubPages[pos + 1]->Underflow()) { // Rotate L2R
...
template <typename Trait> void CBTreePage<Trait>::RedistributeR2L(size_t pos) {
  ...
  while (pSource->GetNumberOfKeys() > pSource->MinNumberOfKeys() &&
    ...
    ::insert_at(pTarget->m_SubPages, pSource->m_SubPages[0],
...
template <typename Trait> void CBTreePage<Trait>::RedistributeL2R(size_t pos) {
  BTPage *pSource = m_SubPages[pos], *pTarget = m_SubPages[pos + 1];
  while (pSource->GetNumberOfKeys() > pSource->MinNumberOfKeys() &&
         pTarget->GetNumberOfKeys() < pSource->GetNumberOfKeys()) {
    ...
    ::insert_at(pTarget->m_SubPages,
                pSource->m_SubPages[pSource->NumberOfKeys()], 0);
    pTarget->NumberOfKeys()++;
...
template <typename Trait> void CBTreePage<Trait>::SplitChild(size_t pos) {
...
template <typename Trait> bool CBTreePage<Trait>::SplitRoot() {
...
void CBTreePage<keyType, ObjIDType>::ForEachReverse(lpfnForEach2 lpfn, size_t
level, void *pExtra1)
{
       if( m_SubPages[m_KeyCount] )
...
template <typename Trait>
void CBTreePage<Trait>::ForEach(lpfnForEach2 lpfn, size_t level,
                                void *pExtra1) {
  for (size_t i = 0; i < m_KeyCount; i++) {
    if (m_SubPages[i])
      m_SubPages[i]->ForEach(lpfn, level + 1, pExtra1);
...
// TODO replace by generic foreach
template <typename Trait>
void CBTreePage<Trait>::ForEach(lpfnForEach3 lpfn, size_t level, void *pExtra1,
                                void *pExtra2) {
  for (size_t i = 0; i < m_KeyCount; i++) {
    if (m_SubPages[i])
      m_SubPages[i]->ForEach(lpfn, level + 1, pExtra1, pExtra2);
...
template <typename Trait>
typename CBTreePage<Trait>::ObjectInfo *
CBTreePage<Trait>::FirstThat(lpfnFirstThat3 lpfn, size_t level, void *pExtra1,
                             void *pExtra2) {
  ObjectInfo *pTmp;
  for (size_t i = 0; i < m_KeyCount; i++) {
    if (m_SubPages[i])
...
template <typename Trait>
bt_ErrorCode CBTreePage<Trait>::Remove(const keyType &key,
                                       const ObjIDType ObjID) {
  bt_ErrorCode error = bt_ok;
  size_t pos = binary_search(m_Keys, 0, m_KeyCount, key);
  if (pos < NumberOfKeys() &&
      key ==
          m_Keys[pos].key /*&& m_Keys[pos].m_ObjID == ObjID*/) // We found it !
  {
    ...
    error = m_SubPages[pos]->Remove(key, ObjID);
  else if (key <= m_Keys[pos].key) // = is because identical keys are inserted
                                   // on left (see Insert)
  {
    if (m_SubPages[pos])
      error = m_SubPages[pos]->Remove(key, ObjID);
...
template <typename Trait> bt_ErrorCode CBTreePage<Trait>::Merge(size_t pos) {
  ...
  BTPage *pChild1 = m_SubPages[pos - 1], *pChild2 = m_SubPages[pos],
...
template <typename Trait> bt_ErrorCode CBTreePage<Trait>::MergeRoot() {
  size_t pos = 1;
  assert(m_SubPages[pos - 1]->NumberOfKeys() + m_SubPages[pos]->NumberOfKeys() +
             m_SubPages[pos + 1]->NumberOfKeys() ==
         3 * m_SubPages[pos]->MinNumberOfKeys() - 1);
...
template <typename keyType, typename ObjIDType>
void Print(tagObjectInfo<keyType, ObjIDType> &info, size_t level,
           void *pExtra) {
...
template <typename Trait> void CBTreePage<Trait>::Print(ostream &os) {
  lpfnForEach2 lpfn = &::Print<keyType, ObjIDType>;
  ForEach(lpfn, 0, &os);
}

template <typename Trait> void CBTreePage<Trait>::Create() {
  Reset();
...
template <typename Trait> void CBTreePage<Trait>::Reset() {
  for (size_t i = 0; i < m_KeyCount; i++)
    delete m_SubPages[i];
  clear();
}

template <typename Trait> void CBTreePage<Trait>::clear() {
  // m_Keys.clear();
  // m_SubPages.clear();
  m_KeyCount = 0;
}
...
template <typename Trait>
void CBTreePage<Trait>::MovePage(BTPage *pChildPage,
                                 vector<ObjectInfo> &tmpKeys,
                                 vector<BTPage *> &tmpSubPages) {
